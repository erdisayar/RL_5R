    ...
    def run(self):
        ...
        for episode in range(self.params["max_num_episodes"]):
            ...
            while not done:
                ...
                # print(action)
                # Before every action , robot is coming to the initial state
                # if (round(next_obs[0],4) != -0.0056) and (round(next_obs[0],4) != 0.2648):
                    # proc = call(["gnome-terminal", "-e","/home/erdi/catkin_ws/src/fiver/fiver_control/src/gazebo_restarter.sh"])
                    # print(proc)
                    # proc2 = subprocess.Popen(['/home/erdi/catkin_ws/src/fiver/fiver_control/src/controller_starter.sh'], shell=True,stdin=None, stdout=None, stderr=None, close_fds=True)
                    # os.system('/home/erdi/catkin_ws/src/fiver/fiver_control/src/gazebo_restarter.sh')
                    # os.system('/home/erdi/catkin_ws/src/fiver/fiver_control/src/controller_starter.sh')

                next_obs, reward, done, _ = self.env.step(action)
                
                print("next,reward,done",next_obs, reward, done)
...
if __name__ == "__main__":
    ...
    RL = DeepActorCriticAgent(agent_params["num_agents"], args.env, agent_params, env_params)
    RL.run()
